...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    cnf = to_cnf(conjoin(premise, ~conclusion))
    if pycoSAT(cnf):
        return False
    return True
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combs = list(itertools.combinations(literals, len(literals) - 1))
    expr = None
    for comb in list(combs):
        if not expr:
            expr = ~disjoin(list(comb))       # all else is false except for one
        else:
            expr = disjoin(expr, ~disjoin(list(comb)))
    return to_cnf(expr)
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    for comb in list(combs):
        if not expr:
            expr = (~disjoin(list(comb))) & list(set(literals)-set(comb))[0]   # one is true, all else is false
        else:
            expr = disjoin(expr, (~disjoin(list(comb))) & list(set(literals)-set(comb))[0])
    return to_cnf(expr)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    return disjoin(possible_causes)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in all_coords:
        has_wall = PropSymbolExpr(wall_str, x, y)
        pacman_at = PropSymbolExpr(pacman_str, x, y, time=t)
        pacphysics_sentences.append(has_wall >> ~pacman_at)

    all_squares = [PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_outer_wall_coords]
    pacphysics_sentences.append(exactlyOne(all_squares))

    all_actions = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
    pacphysics_sentences.append(exactlyOne(all_actions))    
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    #KB.append(pacmanSuccessorAxiomSingle(x1, y1, 1, walls_grid))        # 确保x1_y1是由x_y0通过action0到达的, t should be 1 because we use this function to get the possible last (location, action)
    #KB.append(allLegalSuccessorAxioms(1, walls_grid, non_outer_wall_coords))
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    KB.append(PropSymbolExpr(action0, time=0))
    KB.append(PropSymbolExpr(action1, time=1))

    pacat = PropSymbolExpr(pacman_str, x1, y1, time=1)
    #pacat = conjoin(PropSymbolExpr(pacman_str, x1, y1, time=1), pacmanSuccessorAxiomSingle(x1, y1, 1, walls_grid))
    #not_at = conjoin(~PropSymbolExpr(pacman_str, x1, y1, time=1), allLegalSuccessorAxioms(1, walls_grid, non_outer_wall_coords))
    model1 = findModel(conjoin(conjoin(KB), pacat))
    model2 = findModel(conjoin(conjoin(KB), ~pacat))
    #model2 = findModel(conjoin(conjoin(KB), ~pacat)) if entails(conjoin(KB), ~pacat) else False
    print("model1: ", model1)
    print("model2: ", model2)
